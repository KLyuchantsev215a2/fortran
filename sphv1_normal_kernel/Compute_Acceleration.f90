subroutine Compute_Acceleration(x,Ci,s,acc,PK1,F,Couchy,thichness,table,x_init,cor_W,nabla_W_0_1,nabla_W_0_2,mu,k,eta,vol,YieldStress,betar,gammar,betas,gammas,rho_0,dt,N) 
  IMPLICIT NONE
    ...
    do i=1,N
        do j=1,table(i,1)
                do alpha=1,2  
                    acc(alpha,i)=acc(alpha,i)-vol*(PK1(alpha,1,table(i,j+1))+PK1(alpha,1,i))*nabla_W_0_1(table(i,j+1),i)
                    acc(alpha,i)=acc(alpha,i)-vol*(PK1(alpha,2,table(i,j+1))+PK1(alpha,2,i))*nabla_W_0_2(table(i,j+1),i)
                enddo
        enddo
        do alpha=1,2
