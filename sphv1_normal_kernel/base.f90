...
!ouput:: tensor stress PK1, coordinats, Force 
 IMPLICIT NONE
integer N,i,j,cap_down,cap_up,k1,k2,a,count_section,count_hole,flag!the number of particles that sample the environment
...
real*8 :: Area,m,rho_0,nu,mu,k,eta,YieldStress,gammar,betar,gammas,betas,etaN ! const materials and body
    
real*8 :: dh!indent for calculating the derived kernel through finite differences
real*8 :: pi!const Pi
    
real*8 :: Force,Force_old!Сила 
...
integer, allocatable :: index_hole(:)
    ...
    read (1, 1100) rho_0, T,nu, mu,k,gammar,betar,gammas,betas,eta,YieldStress, dh,N  
    write (*, 1100) rho_0, T,nu, mu,k,gammar,betar,gammas,betas,eta,YieldStress, dh,N
    gammas=-gammas
    pi=3.14159265359
...
!    enddo
 ...
 if(step-int(step/fr)*fr==0) then
        ...
        write (*,1112) Force,x(2,index_hole(1))-x_init(2,index_hole(1))
        xplot(1:2,1:N,coutfr)=x
        coutfr=coutfr+1
    endif
    !вычисление ускорения
    call Compute_Acceleration(x,Ci,s,acc,PK1,F,Cauchy,thichness,table,x_init,cor_W,nabla_W_0_1,nabla_W_0_2,mu,k,eta,vol,YieldStress,betar,gammar,betas,gammas,rho_0,dt,N)
